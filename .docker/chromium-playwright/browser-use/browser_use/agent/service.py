	...
	async def step(self, step_info: Optional[AgentStepInfo] = None) -> None:
		...
		try:
			...
			if self.settings.planner_llm and self.state.n_steps % self.settings.planner_interval == 0:
				...
				logger.info("플래너 실행 완료")

			logger.info("\n4. 입력 메시지 가져오기")
			input_messages = self._message_manager.get_messages()
			logger.info(f"메시지 개수: {len(input_messages)}")

			self._raise_if_stopped_or_paused()

			try:
				logger.info("\n5. LLM에 다음 액션 요청")
				logger.info("=== String Passed to LLM ===")
				for msg in input_messages:
					logger.info(f"\n메시지 타입: {type(msg).__name__}")
					# logger.info(f"메시지 내용: {msg.content[:200]}...")
	...
	async def get_next_action(self, input_messages: list[BaseMessage]) -> AgentOutput:
		...
		try:
			if self.model_name == 'deepseek-reasoner' or self.model_name.startswith('deepseek-r1'):
				...
				logger.info("2. 기본 LLM 처리 시작")
				structured_llm = self.llm.with_structured_output(self.AgentOutput, include_raw=True)
				logger.info("LLM 요청 보내기 직전")
				
				# 타임아웃 추가
				try:
					# asyncio.wait_for로 타임아웃 설정 (30초)
					response: dict[str, Any] = await asyncio.wait_for(
						structured_llm.ainvoke(input_messages), 
						timeout=30.0
					)
					logger.info("LLM 응답 받음:", response)
					parsed: AgentOutput | None = response['parsed']
				except asyncio.TimeoutError:
					logger.error("LLM 요청 타임아웃 (30초)")
					raise TimeoutError("LLM request timed out after 30 seconds")
				except Exception as e:
					logger.error(f"LLM 요청 중 오류 발생: {type(e).__name__}: {str(e)}")
					raise
				
			else:
				logger.info("3. Tool calling 처리 시작")
				...
				try:
					# LLM 모델명과 입력 메시지 디버깅
					logger.info(f"사용 모델: {self.model_name}, 라이브러리: {self.chat_model_library}")
					logger.info(f"입력 메시지 수: {len(input_messages)}")
			...
			if parsed is None:
				logger.info("파싱 실패: parsed is None")
				raise ValueError('Could not parse response.')
			...
			logger.error(f"get_next_action 메서드 실행 중 오류: {type(e).__name__}: {str(e)}")
			raise
